...
const FinalCTASection: React.FC = () => {
  ...
  return (
    <>
      <section id="contact" className="py-24 bg-[#2C3E50] text-white relative overflow-hidden">
        {/* Background decoration */}
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-[#2C3E50] rounded-full mix-blend-multiply filter blur-3xl opacity-20 translate-x-1/2 -translate-y-1/2 z-0"></div>
        ...
        <ScrollReveal>
          <div className="container mx-auto px-6 relative z-10">
            {/* NEW TOP SECTION: Centered Headline & Statute Box */}
            <div className="text-center max-w-4xl mx-auto mb-16">
              <h2 className="text-4xl md:text-5xl font-black mb-10 leading-[1.2]">
                <span className="text-[#EDC967]">서두르세요.</span><br />
                시효가 지나기 전에<br />
                전문가에게 확인하셔야 합니다.
              </h2>
              <div className="bg-[#152e46]/80 backdrop-blur-sm border-l-4 border-[#EDC967] p-6 rounded-r-2xl inline-block text-left max-w-2xl">
                <p className="text-slate-300 text-sm md:text-base leading-relaxed">
                  *<strong className="text-white">시효</strong>: 발생 인지 후 청구해야 하는 기간. <br className="hidden md:block" />
                  후유장해는 <strong className="text-[#EDC967]">180일 이내</strong> 청구해야 합니다. (교통사고·배상책임 기준)
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 items-start">
              ...
              <div className="lg:w-1/2 w-full">
                <div className="space-y-6">
                  <a href="tel:02-6959-5053" className="flex items-center gap-4 bg-[#152e46] p-6 rounded-2xl hover:bg-[#1a3654] transition-colors border border-[#234567] group">
                    <div className="w-12 h-12 bg-[#2C3E50] rounded-full flex items-center justify-center group-hover:scale-110 transition-transform">
                      <Phone className="w-6 h-6 text-[#EDC967]" />
                    </div>
                    <div>
              ...
              <div id="contact-form" className="lg:w-1/2 w-full bg-white rounded-3xl p-8 md:p-10 text-slate-900 shadow-2xl scroll-mt-24">
                <h3 className="text-2xl font-bold mb-6 flex items-center gap-2">
                  <span className="w-2 h-8 bg-[#2C3E50] rounded-full"></span>
                  착수금 없이 상담하기
                </h3>
                ...
                <form className="space-y-5" onSubmit={handleSubmit}>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-[#F9F9F7] border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-[#2C3E50] transition-colors"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {['교통사고', '시설사고·낙상사고', '업무관련사고', '기타'].map((type) => (
                        <label key={type} className="flex items-center gap-2 bg-[#F9F9F7] p-3 rounded-xl border border-slate-200 cursor-pointer hover:bg-[#2C3E50]/10 hover:border-[#2C3E50] transition-colors">
                          <input
                            type="radio"
                            name="accidentType"
                            value={type}
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">장해 진단 여부</label>
                    <div className="grid grid-cols-3 gap-3">
                      {['받음', '예정', '미정'].map((status) => (
                        <label key={status} className="flex items-center gap-2 bg-[#F9F9F7] p-3 rounded-xl border border-slate-200 cursor-pointer hover:bg-[#2C3E50]/10 hover:border-[#2C3E50] transition-colors">
                          <input
                            type="radio"
                            name="disabilityStatus"
                            value={status}
                            checked={disabilityStatus === status}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-[#F9F9F7] border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-[#2C3E50] transition-colors resize-none"
                    ></textarea>
                  </div>
                  ...
                  <label className="flex items-start gap-2 cursor-pointer mt-2">
                    <input
                      ...
                      className="mt-1 text-[#2C3E50] rounded focus:ring-[#2C3E50]"
                    />
                  ...
                  <button
                    ...
                    className={`w-full bg-[#2C3E50] hover:bg-[#1a2530] text-white font-bold py-5 rounded-xl text-base md:text-xl transition-all shadow-[0_0_20px_rgba(44,62,80,0.5)] hover:shadow-[0_0_30px_rgba(44,62,80,0.7)] transform hover:-translate-y-1 mt-4 whitespace-nowrap ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''
                      }`}
                  >
                    {isSubmitting ? '신청 중...' : '착수금 없이 상담하기'}
                  </button>
                </form>
              </div>

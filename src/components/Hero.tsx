  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col justify-center overflow-hidden">
      ...
      <div className="relative container mx-auto px-6 py-20 md:py-32 z-10 flex flex-col h-full justify-center">
        {/* Top Tag */}
        <div className="mb-8 inline-flex items-center space-x-2 text-[#3182A4] font-semibold tracking-wider text-sm md:text-base border-l-4 border-[#3182A4] pl-4 animate-fade-in-up">
          <span>교통사고 합의 전문</span>
        </div>
        ...
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-black leading-tight mb-8 text-white drop-shadow-2xl animate-fade-in-up delay-100">
          교통사고 합의,<br />
          단 한번의 판단으로<br />
          <span className="text-yellow-400 decoration-[#3182A4]/30 underline decoration-8 underline-offset-4">수천만 원</span>을 잃을 수 있습니다.
        </h1>

        {/* Description */}
        <p className="text-lg md:text-2xl text-slate-200 mb-10 max-w-3xl font-light leading-relaxed animate-fade-in-up delay-200">
          걱정하지 마세요.<br />
          <strong className="text-white font-bold">합의에서 소송까지</strong> 한 번에<br className="md:hidden" />
          <strong className="text-white font-bold"> 손해사정사·변호사·노무사</strong> 팀이<br className="hidden md:block" />
          놓치는 보상 없이 찾아드립니다.
        </p>
        ...
        <div className="mb-16 animate-fade-in-up delay-300 flex justify-center md:justify-start">
          <a
            href="#contact-form"
            className="group inline-flex items-center justify-center gap-3 bg-yellow-400 hover:bg-yellow-300 text-[#1B3B5A] px-10 py-5 rounded-2xl font-black text-xl md:text-2xl transition-all shadow-[0_0_20px_rgba(250,204,21,0.4)] hover:shadow-[0_0_30px_rgba(250,204,21,0.6)] transform hover:-translate-y-1 whitespace-nowrap w-full md:w-auto"
          >
            착수금 0원으로 시작하기
        ...
        <div className="animate-fade-in-up delay-500 max-w-4xl">
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-12 relative z-10">
              {[
                { label: '변호사', icon: Scale, desc: '합의·소송 전담' },
                { label: '손해사정사', icon: Calculator, desc: '보상금 산정' },
                { label: '노무사', icon: Briefcase, desc: '산재 보상 전문' }
              ].map((item, i) => (
                <div key={i} className="flex flex-row md:flex-col items-center gap-4 md:gap-2 bg-white/5 backdrop-blur-xl border border-white/10 p-5 md:p-6 rounded-2xl group hover:bg-white/10 transition-all hover:-translate-y-1">
                  <div className="w-12 h-12 md:w-16 md:h-16 bg-[#3182A4]/20 rounded-2xl flex items-center justify-center mb-0 md:mb-2 group-hover:bg-[#3182A4]/40 transition-colors">
                    <item.icon className="w-6 h-6 md:w-8 md:h-8 text-[#3182A4]" />
                  </div>
          ...
          <p className="mt-8 text-slate-300 text-sm md:text-base flex items-center gap-2 font-medium">
            <CheckCircle2 className="w-5 h-5 text-[#3182A4]" />
            세 분야의 전문가가 유기적으로 협력하여 최선의 결과를 만듭니다.
          </p>
        </div>
      </div>

      {/* Decorative Gradient Bottom */}
      <div className="absolute bottom-0 left-0 w-full h-24 bg-gradient-to-t from-[#F4F7F9] to-transparent z-10"></div>
    </div>
  );
};
